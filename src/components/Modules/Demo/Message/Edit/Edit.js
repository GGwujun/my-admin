...
module.exports = {
	...
	methods: {
		...
		onSubmit(formName) {
			...
			ref.validate((valid) => {
				if (valid) {
					...
					this.article_data.user = 'dsx';
					this.article_data.password = '123321';

					let ops = {
						cbFn: null,
						tokenFlag: null,
						errFn: null,
						host: 'http://114.67.135.113:3002/util/push_message'
					}
					this.$$api_message_saveMessage(this.article_data, data => {
						this.$router.push('/demo/message/list');
					}, ops);
				}
			});
		},
		uploadSubmit(file) {
			this.uploadFile(file.file, (response) => {
				console.log()
				console.log('uploadFile success= ', )
			})
		},
		getToken(file, complete) {
			if (!file.name) {
				alert('小提示', '图片没有名称哦~')
				return
			}
			...
			let ops = {
				...
				tokenFlag: null,
				errFn: null,
				host: 'http://114.67.135.113:3002/util/qiniu_token'
			}
			if ((!this.token) || (!this.token.token)) {
				this.$$api_message_saveMessage({ filename: file.name }, response => {
					if (response.status == 0) {
						file.token = response.qiniu_token;
						this.token = { token: response.qiniu_token, key: file.name }
					}
				}, ops);
			}
		},
		...
		uploadSuccess(response, file, fileList) {
			this.article_data.image = 'http://p1aewhbrc.bkt.clouddn.com/' + response.key;
			this.token = null;
		},
