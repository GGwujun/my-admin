...
module.exports = {
	...
	methods: {
		 ...
		 */
		onDeleteArticle(classs, index, list) {
			if (classs === true) {
				var _id = this.batch_id;
			} else {
				var _id = classs._id;
			}

			this.$$api_class_deleteClass({
				id: _id
			}, (data) => {
				if (classs === true) {
					this.article_list = this.article_list.filter(function (item, idx) {
						return _id.indexOf(item._id) === -1;
					});
				} else {
					list.splice(index, 1);
				}
		 ...
		 */
		onEditArticle(classs) {
			if (classs && classs._id) {
				this.$router.push('/demo/class/edit?id=' + classs._id);
			} else {
				this.$message({
					showClose: true,
					message: 'ID跑哪去了？',
		...
		/**
		 * 添加子课程
		 * @param {*object} 当前课程信息对象 
		 */
		onAddChild(classs) {
			if (classs && classs._id) {
				this.$router.push('/demo/class/edit?pid=' + classs.title);
			} else {
				this.$message({
					showClose: true,
					message: 'ID跑哪去了？',
		...
		getList({
			...
			this.$$api_class_selectClass(data, (article_data) => {
